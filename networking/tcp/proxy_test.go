...
func proxyConn(source, destination string) error {
	...
	// Handle replies from destination to source
	go func() {
		_, _ = io.Copy(connSource, connDestination)
	}()

	// connSource messages connDestination
	_, err = io.Copy(connDestination, connSource)

	return err
}

// Takes general Reader and Writer Interfaces
// Proxy data to -> os.StdOut, *bytes.Buffer, *os.File, etc
...
func TestProxy(t *testing.T) {
		...
		for {
			...
			go func(c net.Conn) {
				...
				for {
					...
					// Server listens for a "ping", and responds with "pong"
	...
	// Proxy Server
	// listens to Client connections, on accepting
	// establishes Destination Server connection and proxies msg
	// Replies from the Destination Server are proxied back (proxy func defined above)
